	...
	const getWithinWeekTask = () => {
		...
		for (const task of accept_list) {
			deadline_list.push({
				...
				day: task.deadline.substring(task.deadline.indexOf('/') + 1, task.deadline.length),
			});
		}
		...
		const findLoomingTask = () => {
			...
			for (const date of deadline_list) {
				...
				if (date.month === today.month) {
					// 日にちの減算結果が7以下であれば納期を配列に入れる
					if (0 <= date.day - today.day && date.day - today.day <= 7)
						date_list.push({
							deadline: date,
					...
					if (date.month - today.month === 1 || date.month - today.month === 11) {
						// 今月の日付最大値と今日の日付の減算結果に納期の日付を加えたものが7以下であれば納期を配列に入れる
						if (0 <= max_days[today.month - 1] - today.day + Number(date.day) && max_days[today.month - 1] - today.day + Number(date.day) <= 7)
							date_list.push({
								deadline: date,
								remain: max_days[today.month - 1] - today.day + Number(date.day),
							});
					}
				}
			}
